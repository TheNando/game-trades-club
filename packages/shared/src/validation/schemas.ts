...
const optionalInteger = (fieldName: string, min: number, max: number) =>
  z
...
export const createListingSchema = z.object({
  ...
  game_id: optionalInteger('game_id', 1, Number.MAX_SAFE_INTEGER)
    .refine((value) => value !== null && value !== undefined, {
      message: 'game_id is required',
    })
    .transform((value) => value!),
  condition: z.enum(CONDITION_VALUES),
  price: optionalInteger('price', 0, MAX_LISTING_PRICE_CENTS)
    .refine((value) => value !== null && value !== undefined, {
      message: 'price is required',
    })
    .transform((value) => value!),
  status: z.enum(listingStatusValues),
  preferred_shop_id: optionalTrimmedText(MAX_SHOP_TEXT_LENGTH).transform((value) => value ?? null),
});
...
export const updateListingSchema = z
  .object({
    ...
    preferred_shop_id: optionalTrimmedText(MAX_SHOP_TEXT_LENGTH),
  })
  .refine(
    (body) =>
...
export const createConversationSchema = z.object({
  recipient_id: requiredTrimmedText('recipient_id', MAX_SHOP_TEXT_LENGTH),
  listing_id: optionalTrimmedText(MAX_SHOP_TEXT_LENGTH),
...
export const listingQuerySchema = z.object({
  ...
  user_id: z.string().trim().min(1).max(MAX_SHOP_TEXT_LENGTH).optional(),
  status: z.enum(listingStatusValues).optional(),
  price_min: optionalInteger('price_min', 0, MAX_LISTING_PRICE_CENTS),
  price_max: optionalInteger('price_max', 0, MAX_LISTING_PRICE_CENTS),
  year_min: optionalInteger('year_min', 0, 10_000),
  year_max: optionalInteger('year_max', 0, 10_000),
  players: optionalInteger('players', 1, 100),
  playtime: optionalInteger('playtime', 1, 10_000),
