...
/** Renders the directory and map of pickup shops. */
        ...
        {loading ? (
          ...
          <>
            {mapPoints.length > 0 ? (
              <ShopMap
                points={mapPoints}
                className="h-80 w-full rounded-2xl border border-base-300"
              />
            ) : (
            ...
            {shopsWithoutCoords > 0 && mapPoints.length > 0 ? (
              <p class="text-xs text-base-content/55">
                {shopsWithoutCoords} {shopsWithoutCoords === 1 ? 'shop is' : 'shops are'} missing
                coordinates and not shown on the map.
              </p>
            ) : null}
                ...
                <div key={group.city}>
                  ...
                  <ul class="mt-3 grid gap-3 sm:grid-cols-2">
                    {group.shops.map((shop) => (
                      <li
                        key={shop.id}
                        class="rounded-2xl border border-base-300 bg-base-100 shadow-sm p-4"
                      >
                        ...
                        {shop.state || shop.zip ? (
                          <p class="text-sm text-base-content/70">
                            {formatStateZip(shop.state, shop.zip)}
                          </p>
                        ) : null}
                        {shop.website_url ? (
                          <a
