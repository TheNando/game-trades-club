...
/** Renders the admin interface for managing pickup shops. */
...
function AdminConsole() {
  ...
  const handleShopUpdated = (updated: Shop) => {
    setShops((existing) =>
      existing.map((s) => (s.id === updated.id ? updated : s)).sort(compareShops),
    );
  };
  ...
  return (
    <div class="min-h-screen bg-base-100 text-base-content">
      ...
      <section class="max-w-4xl mx-auto px-4 md:px-8 py-10 grid gap-10 md:grid-cols-[1fr_1fr]">
        <form
          class="rounded-2xl border border-base-300 bg-base-100 shadow-sm p-6 md:p-7 flex flex-col gap-4"
          onSubmit={submitShop}
        >
          <h2 class="font-display text-2xl">Add a shop</h2>
          <div class="flex flex-col gap-1.5">
            <label class="text-sm font-medium" for="shop-name">
              Name
            </label>
            <input
              id="shop-name"
          ...
          <div class="flex flex-col gap-1.5">
            <label class="text-sm font-medium" for="shop-address">
              Address
            </label>
            <input
              id="shop-address"
          ...
          <div class="flex flex-col gap-1.5">
            <label class="text-sm font-medium" for="shop-city">
              City
            </label>
            <input
              id="shop-city"
          ...
          <div class="grid gap-3 sm:grid-cols-2">
            <div class="flex flex-col gap-1.5">
              <label class="text-sm font-medium" for="shop-state">
                State
              </label>
              <input
                id="shop-state"
            ...
            <div class="flex flex-col gap-1.5">
              <label class="text-sm font-medium" for="shop-zip">
                ZIP
              </label>
              <input
                id="shop-zip"
          ...
          <div class="flex flex-col gap-1.5">
            <label class="text-sm font-medium" for="shop-website">
              Website
            </label>
            <input
              id="shop-website"
          ...
          <div class="grid gap-3 sm:grid-cols-2">
            <div class="flex flex-col gap-1.5">
              <label class="text-sm font-medium" for="shop-latitude">
                Latitude
              </label>
              <input
                id="shop-latitude"
            ...
            <div class="flex flex-col gap-1.5">
              <label class="text-sm font-medium" for="shop-longitude">
                Longitude
              </label>
              <input
                id="shop-longitude"
            ...
            <p class="sm:col-span-2 text-xs text-base-content/55">
              On Google Maps, right-click the shop's location and click the coordinates at the top
              of the menu to copy them. Paste the first number into Latitude and the second into
              Longitude. Optional, but required to show the shop on the map.
            </p>
          </div>
    ...
    return (
      <li class="rounded-2xl border border-base-300 bg-base-100 shadow-sm p-4">
        <form class="flex flex-col gap-3" onSubmit={saveEdit}>
          <div class="flex flex-col gap-1.5">
            <label class="text-xs font-medium" for={`edit-name-${shop.id}`}>
              Name
            </label>
            <input
              id={`edit-name-${shop.id}`}
          ...
          <div class="flex flex-col gap-1.5">
            <label class="text-xs font-medium" for={`edit-address-${shop.id}`}>
              Address
            </label>
            <input
              id={`edit-address-${shop.id}`}
              class="input input-bordered input-sm rounded-lg"
              type="text"
              value={address}
              onInput={(e) => setAddress((e.currentTarget as HTMLInputElement).value)}
          ...
          <div class="flex flex-col gap-1.5">
            <label class="text-xs font-medium" for={`edit-city-${shop.id}`}>
              City
            </label>
            <input
              id={`edit-city-${shop.id}`}
              class="input input-bordered input-sm rounded-lg"
              type="text"
              required
              value={city}
          ...
          <div class="grid gap-3 sm:grid-cols-2">
            <div class="flex flex-col gap-1.5">
              <label class="text-xs font-medium" for={`edit-state-${shop.id}`}>
                State
              </label>
              <input
                id={`edit-state-${shop.id}`}
                class="input input-bordered input-sm rounded-lg"
                type="text"
                value={stateValue}
            ...
            <div class="flex flex-col gap-1.5">
              <label class="text-xs font-medium" for={`edit-zip-${shop.id}`}>
                ZIP
              </label>
              <input
                id={`edit-zip-${shop.id}`}
          ...
          <div class="flex flex-col gap-1.5">
            <label class="text-xs font-medium" for={`edit-website-${shop.id}`}>
              Website
            </label>
            <input
              id={`edit-website-${shop.id}`}
              class="input input-bordered input-sm rounded-lg"
              type="text"
              value={websiteUrl}
              onInput={(e) => setWebsiteUrl((e.currentTarget as HTMLInputElement).value)}
          ...
          <div class="grid gap-3 sm:grid-cols-2">
            <div class="flex flex-col gap-1.5">
              <label class="text-xs font-medium" for={`edit-latitude-${shop.id}`}>
                Latitude
              </label>
              <input
                id={`edit-latitude-${shop.id}`}
                class="input input-bordered input-sm rounded-lg"
                type="text"
                inputMode="decimal"
                value={latitude}
            ...
            <div class="flex flex-col gap-1.5">
              <label class="text-xs font-medium" for={`edit-longitude-${shop.id}`}>
                Longitude
              </label>
              <input
                id={`edit-longitude-${shop.id}`}
                class="input input-bordered input-sm rounded-lg"
                type="text"
                inputMode="decimal"
                value={longitude}
  ...
  return (
    <li class="rounded-2xl border border-base-300 bg-base-100 shadow-sm p-4">
      ...
      {shop.latitude !== null && shop.longitude !== null ? (
        <p class="mt-1 text-xs text-base-content/55 font-mono">
      ...
      {deleteError ? (
        <div class="alert alert-error rounded-xl mt-2">
