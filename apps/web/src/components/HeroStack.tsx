...
function getRandomCards(cards: typeof HERO_CARDS, count: number): typeof HERO_CARDS {
  ...
  return shuffled.slice(0, count).map((card, index) => ({ ...card, accent: colors[index % colors.length] }));
}
    ...
    >
      <div className='p-4'>
        ...
        <div class="mt-3 flex items-baseline justify-between gap-2">
          <p
            class="font-display text-lg leading-tight truncate transition-opacity"
          >{title}</p>
